  ...
  return (
    <section className="bg-[#F8F4FF] py-16 lg:py-20">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        <span className="inline-flex items-center gap-2 bg-[#FFF0F9] text-[#fc66b5] text-xs font-bold px-4 py-1.5 rounded-full mb-6 tracking-widest uppercase">
        ...
        <div className="flex flex-wrap justify-center gap-3">
          {[
            ...
            <span key={b.text} className="bg-[#fc66b5] text-white text-xs font-bold px-5 py-2 rounded-full flex items-center gap-2">
              <span>{b.icon}</span>
              {b.text}
            </span>
          ))}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16 items-center">
          ...
          <div className={`w-full lg:flex-1 ${reverse ? 'lg:order-1' : 'lg:order-2'}`}>
            ...
            <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 mb-2">{esp.nombre}</h2>
            <p className={`text-sm font-semibold mb-6 ${esp.badgeColor.split(' ')[1]}`}>{esp.cargo}</p>
            <p className="text-gray-500 text-sm leading-relaxed mb-7">{esp.bio}</p>

            {/* Tags */}
            <div className="flex flex-wrap gap-2 mb-8">
              {esp.tags.map((tag, i) => (
                <span key={tag} className={`text-xs font-bold px-3 py-1.5 rounded-full ${esp.tagColors[i]}`}>
                  {tag}
                </span>
              ))}
            </div>
    ...
    <section className="bg-[#1A1035] py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <span className="text-xs font-bold tracking-widest text-[#fc66b5] uppercase mb-3 block">Nuestra Filosofía</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {filosofia.map((f) => (
            <div
              key={f.title}
              className={`rounded-2xl p-8 bg-gradient-to-br ${f.gradient} text-white`}
            >
